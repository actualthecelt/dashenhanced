...
if selectuser == "1":
    ...
    statement = f"SELECT username from user1_dep2 WHERE password='{passinput}';"
    ...
    if not cur.fetchone():
        ...
        log.close()
        t.sleep(3)
        subprocess.call(r"python C:\Users\Celt\dashenhanced\main\deps\dep2\users\user1.py 1", shell=True)
elif selectuser == "2":
    ct()
    print("LOGIN PAGE\n\nSelected user: User2")
    passinput = gp.getpass("Password:")
    statement = f"SELECT username from user2_dep2 WHERE password='{passinput}';"
    cur.execute(statement)
    if not cur.fetchone():
        ct()
        print(bcolors.FAIL + "Incorrect password." + bcolors.RESET)
        ...
        ct()
        print(bcolors.OK + "Access granted.\n" + bcolors.RESET + "Redirecting you in 3 seconds...")
        log = open(r"C:\Users\Celt\dashenhanced\logging\userlogs\dep2\user2_logs.txt", "a")
        log.write("" + currenttime + " - User2 was successfully logged into \n")
        log.close()
        t.sleep(3)
        subprocess.call(r"python C:\Users\Celt\dashenhanced\main\deps\dep2\users\user2.py 1", shell=True)
elif selectuser == "3":
    ct()
    print("LOGIN PAGE\n\nSelected user: User3")
    passinput = gp.getpass("Password:")
    statement = f"SELECT username from user3_dep2 WHERE password='{passinput}';"
    cur.execute(statement)
    if not cur.fetchone():
        ct()
        print(bcolors.FAIL + "Incorrect password." + bcolors.RESET)
        ...
        ct()
        print(bcolors.OK + "Access granted.\n" + bcolors.RESET + "Redirecting you in 3 seconds...")
        log = open(r"C:\Users\Celt\dashenhanced\logging\userlogs\dep2\user3_logs.txt", "a")
        log.write("" + currenttime + " - User3 was successfully logged into \n")
        log.close()
        t.sleep(3)
        subprocess.call(r"python C:\Users\Celt\dashenhanced\main\deps\dep2\users\user3.py 1", shell=True)
else:
    ct()
